            ...
            <div className="flex flex-col">
              {notifications.slice(0, 10).map((notification: any, index: number) => (
                <div key={notification._id || notification.id || index} className="border-b border-border/50 last:border-0">
                  <DropdownMenuItem className={`p-3 cursor-pointer items-start gap-3 focus:bg-accent/50 group transition-colors rounded-none border-l-2 ${
                    !notification.isRead 
                      ? 'border-l-primary bg-primary/[0.03] dark:bg-primary/[0.06]' 
                    ...
                    <div className="flex flex-col gap-1 w-full min-w-0">
                      <div className="flex items-center justify-between w-full gap-2">
                        <div className="flex items-center gap-1.5 min-w-0">
                          <span className={`text-sm font-semibold truncate ${!notification.isRead ? 'text-foreground font-bold' : 'text-muted-foreground'}`}>
