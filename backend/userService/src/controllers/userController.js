...
function getClientBaseUrl(req, client = "public") {
  ...
  if (host) {
    ...
    return client === "admin" ? `${base}/admin` : base;
  }

  // Fallback: use only the first URL from the comma-separated env var
  const envUrl =
    client === "admin"
...
exports.register = asyncHandler(async (req, res, next) => {
  ...
  const verifyUrl = `${getClientBaseUrl(req)}/verify-email/${rawToken}`;
...
exports.forgotPassword = asyncHandler(async (req, res, next) => {
  ...
  const resetUrl = `${getClientBaseUrl(req, client)}/reset-password/${rawToken}`;
...
exports.requestEmailUpdate = asyncHandler(async (req, res, next) => {
  ...
  const verifyUrl = `${getClientBaseUrl(req, client)}/verify-email-update/${rawToken}`;
 ...
 * @route   PATCH /api/v1/auth/deactivate-me
 * @access  Private
 */
...
/**
 * @desc    Get public profile (for instructor info in courses)
 * @route   GET /api/v1/auth/public/:id
 * @access  Public
 */
exports.getPublicProfile = asyncHandler(async (req, res, next) => {
  const user = await User.findById(req.params.id).select('profile role email');
